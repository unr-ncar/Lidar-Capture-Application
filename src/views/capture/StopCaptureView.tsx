...
export default function StopCaptureView() {
    ...
    const rosSensorSelections = displayedSelections.map((selectionItem) => {
        return <SensorSelectionItem key={selectionItem.item.lidar_id} operation='stop' selected={() => isSelected(selectionItem.item.lidar_id, "ros")} toggleFunction={() => toggleSelection(selectionItem.item.lidar_id, "ros")} format="ros" lidarMetadata={selectionItem.item} />
    })

    const pcapSensorSelections = displayedSelections.map((selectionItem) => {
        return <SensorSelectionItem key={selectionItem.item.lidar_id} operation='stop' selected={() => isSelected(selectionItem.item.lidar_id, "pcap")} toggleFunction={() => toggleSelection(selectionItem.item.lidar_id, "pcap")} format="pcap" lidarMetadata={selectionItem.item} />
    })

    return (
        <>
            <Pane minimalWidth>
                <PaneSection label="Sensor Selections for Stopping Active Capture Job" description="Select which sensors and their respective capture services for stopping an active capture job.">
                    <div>
                        <ItemList>
